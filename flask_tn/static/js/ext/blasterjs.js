...
module.exports = blasterjs = Class(
    /** @lends Biojs.blasterjs# */
    {
		...
		buttonDownloadPNG.style.marginRight = '10px';
                ...
                let desc = alignment.description.trim();
                if(desc.startsWith("ISFinder") || desc.startsWith("INTEGRALL")){
                    span.innerHTML         = desc;
                }else{
                    let exists = /^.+\|\d+/.test(desc);
                    if(exists){
                        let s_split = desc.split('|');
                        let rank = s_split.pop();
                        let acc = s_split.join('|');
                        acc = acc.replace(/\s/g, '');
                        span.innerHTML = `${desc}&nbsp;<a href="/report/gene/${acc}/${rank}" target='_blank' class='btn btn-sm btn-warning'>Gene Report</a>`;
                    }else{
                        span.innerHTML = `${desc}&nbsp;<a href="/report/te/${desc}" target='_blank' class='btn btn-sm btn-warning'>TE Report</a>`;
                    }
                }
