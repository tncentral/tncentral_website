            ...
            /** @lends Biojs.blasterjs# */
            {
                ...
                _displayAlignments: function (evt, self) {
                    ...
                    function createSingleAlignment(alignment, self, hsps, hsp) {
                        ...
                        // console.log(alignment.description);
                        // console.log(alignment.hsp[hsp]);
                        ...
                        buttonDownloadJPEG.innerHTML = 'Download as Fasta';
                        buttonDownloadJPEG.onclick = function () { downloadSingleAlignmentFasta("Query", alignment.description, alignment.hsp[hsp]); };
                        if (alignment.description.startsWith("ISFinder") || alignment.description.startsWith("INTEGRALL")) {
                            span.innerHTML = alignment.description;
                        } else {
                            let exists = /^.+\|\d+/.test(alignment.description);
                            if (exists) {
                                let s_split = alignment.description.split('|');
                                let rank = s_split.pop();
                                let acc = s_split.join('|');
                                acc = acc.replace(/\s/g, '');
                                span.innerHTML = `${alignment.description}&nbsp;<a href="/report/gene/${acc}/${rank}" target='_blank' class='btn btn-sm btn-warning'>Gene Report</a>`;
                            } else {
                                span.innerHTML = `${alignment.description}&nbsp;<a href="/report/te/${alignment.description}" target='_blank' class='btn btn-sm btn-warning'>TE Report</a>`;
                            }
                        }
                    ...
                    function downloadSingleAlignmentFasta(query_name, subject_name, hsp) {
                        var a = window.document.createElement('a');
                        var lines = [];
                        header_fasta = `>${query_name}_${hsp.queryStart}_${hsp.queryEnd}`;
                        lines.push(header_fasta+"\n");
                        lines.push(hsp.query+"\n");
                        header_fasta = `>${subject_name}_${hsp.subjectStart}_${hsp.subjectEnd}`;
                        lines.push(header_fasta+"\n");
                        lines.push(hsp.subject+"\n");
                        a.href = window.URL.createObjectURL(new Blob(lines, { type: 'text' }));
                        var filename = `${query_name}_${hsp.queryStart}_${hsp.queryEnd}.fa`;
                        a.download = filename;
